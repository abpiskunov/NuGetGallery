...
{
    /// <summary>
    /// Ported, framework-independent copy of the legacy
    /// <c>NuGetGallery.AsyncFileUpload.AsyncFileUploadRequestParser</c>
    /// (src/NuGetGallery.Services/Authentication/AsyncFileUpload/AsyncFileUploadRequestParser.cs). This class
    /// never actually depended on System.Web -- it is a hand-rolled incremental multipart/form-data scanner
    /// operating purely on byte buffers (Knuth-Morris-Pratt boundary search) so it can extract the
    /// currently-uploading file's name from its Content-Disposition header as bytes stream past, without
    /// buffering the whole request body. The logic is unchanged from the legacy implementation; only the
    /// namespace/location moved because the original lives in NuGetGallery.Services, which has no net10.0
    /// target yet (see AsyncFileUploadProgressMiddleware for how this is now driven from Core middleware
    /// instead of an IHttpModule).
    /// </summary>
    internal class AsyncFileUploadRequestParser
    {
        ...
        private readonly List<byte> _contentDisposition = new List<byte>(200);

        private readonly byte[] _newLine = new byte[] { 13, 10 };
                ...
                if (buffer[i] == _newLine[_newLineIndex])
                {
                    ...
                    if (_newLineIndex == 2)
                    {
                        ...
                        else
                        {
                            ...
                            CurrentFileName = ParseContentDisposition(_contentDisposition.ToArray());
        ...
        private void SearchForBoundary(byte[] text, int startIndex, int lastIndex)
        {
            ...
            while (j < lastIndex)
            {
                ...
                if (i >= _boundary.Length)
                {
                    ...
                    if (_previousBoundaryStartIndex > -1 && CurrentFileName != null)
                    {
                        int previousFileSize = thisBoundaryStartIndex - _previousBoundaryStartIndex - _boundary.Length;
                        System.Diagnostics.Debug.Assert(previousFileSize >= 0);
                    }
